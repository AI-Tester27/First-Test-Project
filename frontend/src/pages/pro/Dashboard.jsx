...
import { Loader2, ChevronRight, ReceiptText } from "lucide-react";
...
export default function ProDashboard() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await api.get("/cases?status=READY_FOR_BILLING,PAYMENT_PENDING,PARTIALLY_PAID");
        setCases(data.cases);
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto" data-testid="pro-dashboard">
      <div className="mb-8">
        <div className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-1">Billing</div>
        <h1 className="font-display text-3xl font-semibold tracking-tight text-gray-900">Billing queue</h1>
      </div>
